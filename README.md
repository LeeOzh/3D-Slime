...
- 程序化身体贴图（白基渐变 + 高光 + 底部阴影 + 轻噪点）
- 角色风味：布丁焦糖滴落、猫软纹、哪吒金点、水滴高光带、圆宝中心光
- `material.color` 仍由角色/色调驱动，软体 GPU 形变不变

### Walk 模式

- 默认捏捏；点 **「散步」** 或 PC 按方向键进入
- **森林小径**（主，移动端仅此场景）：竹林/树/草地，自动向前，左右转向
- PC 另可选：软糖草地、果冻厨房、云上小岛
- 移动：左下虚拟方向键；`Esc` / 返回捏捏退出
- 走路时 UI 自动隐藏；相机固定远景
